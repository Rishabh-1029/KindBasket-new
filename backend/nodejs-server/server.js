...
mongoose
  ...
  .then(() => console.log("KindBasket Account DB connected"))
  .catch((err) => {
    console.error("KindBasket Account DB connection error:", err);
...
app.post("/donate", verifyToken, (req, res) => {
  const { name, email, donationDate, location, city } = req.body;
  const userId = req.userId;
  if (!name || !email || !donationDate || !location || !city) {
    return res
      .status(400)
